...
async def commit(_, message):
    await message.reply_text((await random_line('Sehid/txt/sehid.txt')), reply_markup=button)


@app.on_callback_query(filters.regex("deyis"))
async def deyis(_, query: CallbackQuery):
    await query.edit_message_text((await random_line('Sehid/txt/sehid.txt')), reply_markup=button)