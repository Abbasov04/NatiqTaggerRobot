...
async def ForceSub(bot: Client, event: Message):
    ...
    try:
        invite_link = await bot.create_chat_invite_link(chat_id=(int(Config.KANAL)))
    except FloodWait as e:
        await asyncio.sleep(e.x)
        fix_ = await ForceSub(bot, event)
        return fix_
    except Exception as err:
        print(f"{Config.KANAL} Kanalına Məcburi Abunə Olmaq Mümkün Deyil\n\nXəta: {err}\n\nSahibimlə: 👨‍💻 @{Config.OWNER_NAME} Əlaqə Saxla")
        return 200
    try:
        user = await bot.get_chat_member(chat_id=(int(Config.KANAL)), user_id=event.from_user.id)
